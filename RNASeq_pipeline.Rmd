...
Salmon is a tool for wicked-fast transcript quantification from RNA-seq data. It requires a reference transcriptome to "map" reads with. This snakemake pipeline also makes use of the reference genome for exra quality control. If you don't have the reference genome, let me know and I can modify the snakefile to run with just the transcriptome. Read more here: https://salmon.readthedocs.io/en/latest/salmon.html#using-salmon
...
Create a file called `references.txt` in teh diretory `snakemake_dir`. This file contains the reference genome and reference transciptome for your species. The first line should contain the genome and the second line should contain the transcriptome or file with the RNA fasta sequences. It is very important that they follow this order. Example:
```{bash eval=FALSE}
...
Make sure that the snakemake file (`snakefile`) is in the directory `snakemake_dir`.
...
snakemake --snakefile snakefile --printshellcmd --jobs 10 --conda-frontend conda
...
This is the content for snakefile2:
